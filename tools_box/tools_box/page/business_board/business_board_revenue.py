...
def get_months(year):
    ...
    centers = ["Baker's Choice", "Graceco Foods", "Princess Snacks"]
    for n, center in enumerate(centers):
        for month in months:
            calendar_start = f"{int(year)}-{int(month)}-01"
            calendar_end = f"{int(year)}-{int(month)}-31"
            # if month in [4, 6, 9, 11]:
            data[n].append(execute(company="Graceco Limited", from_date=calendar_start, to_date=calendar_end, item_group=centers[n]))

    total = [sum(data[0]), sum(data[1]), sum(data[2])] #bakers_choice, graceco_foods, princess_snacks = sum(data[0]), sum(data[1]), sum(data[2])
    for i, j in enumerate(data):
        new_data[i].append([round(n/1000000, 3) for n in data[i]])

    return {'data':new_data, 'total': total} #bakers_choice, graceco_foods, princess_snacks


def execute(company, from_date, to_date, item_group):
    filters = {'company': company, 'from_date': from_date, 'to_date': to_date, 'item_group': item_group}
