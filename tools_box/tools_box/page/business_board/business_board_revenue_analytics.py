...
@frappe.whitelist()
def get_territory_sales_analytics(year):
    filters = {
        'tree_type':'Territory', 'from_date': f'{int(year)}-01-01', 'to_date': f'{int(year)}-12-31', 'company': 'Graceco Limited',
         'value_quantity': 'Value', 'range':'Monthly', 'doc_type':'Sales Invoice',
        }

    # get territory
    territory_index = 39, 2, 27, 16, 21, 34 #, 1, 61 # lagos, sw, ss, se, nz, inter, NG, MM, location index
    territories = []
    data = execute(filters=filters)[1]
    for i in territory_index:
        territories.append(data[i]['total'])
    total = sum(territories)
    territory_list = [round(i/1000000000, 3) for i in territories] 
    return territory_list,  total #, execute(filters=filters)[1]



@frappe.whitelist()
def get_analytics_by_month(year1, year2):
    ...
    def get_year_data(year):
        ...
        filters = {
            'tree_type':'Item Group', 'from_date': f'{int(year)}-01-01', 'to_date': f'{int(year)}-12-31', 'company': 'Graceco Limited',
             'value_quantity': 'Value', 'range':'Monthly', 'doc_type':'Sales Invoice',
            }
        total_revenue = [] #{'bakers_choice': 0, 'princess_snacks': 0, 'graceco_foods': 0}
        bakers_choice, princess_snacks, graceco_foods = [], [], []
        chart_data = []
    
        for key, value in execute(filters=filters)[1][1].items():
            chart_data.append(value)
        total_revenue.append(chart_data[-1])
        chart_data = [round(i/1000000, 3) for i in chart_data[2:-1]]           

        #monthly_chart_zip_data, monthly_chart_zip_data2 = zip(chart_data[0], chart_data[1], chart_data[2]), deepcopy(zip(chart_data[0], chart_data[1], chart_data[2])) 
        #monthly_data = [round(sum(i)/1000000, 3) for i in chart_data]
        data = {"monthly_data": chart_data, 'revenue': total_revenue} #monthly_data, 'revenue': sum(total_revenue)}
        return data

    data = []
    for x, year in enumerate(year_list):
        data.append(get_year_data(year))
     
    #data = {"monthly_data": monthly_data, 'revenue': sum(total_revenue)}
    return data 
...
@frappe.whitelist()
def get_analytics_by_revenue_center(year):
    filters = {
        'tree_type':'Item Group', 'from_date': f'{int(year)}-01-01', 'to_date': f'{int(year)}-12-31', 'company': 'Graceco Limited',
         'value_quantity': 'Value', 'range':'Monthly', 'doc_type':'Sales Invoice',
        }
    total_revenue = [] #{'bakers_choice': 0, 'princess_snacks': 0, 'graceco_foods': 0}
    bakers_choice, princess_snacks, graceco_foods = [], [], []
    chart_data = [bakers_choice, princess_snacks, graceco_foods]
    for i, j in enumerate(chart_data):
        for key, value in execute(filters=filters)[1][i+2].items():
            chart_data[i].append(value)
        total_revenue.append(chart_data[i][-1])
        chart_data[i] = chart_data[i][2:-1]           

    for n, m in enumerate(chart_data):
        chart_data[n] = [round(chart_data_col_value/1000000, 3) for chart_data_col_value in chart_data[n]]
    #bakers_choice_data = execute(filters=filters)[1][2]
    
    #for key, value in bakers_choice_data.items():
    #    bakers_choice.append(value)
    #bakers_choice = bakers_choice[2:-1]
    
    data = {"bakers_choice": chart_data[0], "princess_snacks": chart_data[1], "graceco_foods": chart_data[2], 'revenue': total_revenue}
    return data 
...
class Analytics(object):
	def __init__(self, filters=None):
		...
		#self.filters.tree_type = 'Item Group'
